...
string trc(const vector<vector<vector<int>>>& matrix,
    ...
    vector<int> info = matrix[i-1][j-1];
    int x = 0;
    while (info[x]!=value) {
        x+=4;
    }
    int left_value = info[x+1];
    int right_value = info[x+2];
    int bracket = info[x+3];
...
int main(){
    ...
    vector<vector<vector<int>>> values_table(
        puzzle_size, vector<vector<int>>(
        puzzle_size, vector<int>()));
    ...
    for(int i=0; i < puzzle_size -1; i++) {
        values_table[i][i+1].push_back(matrix[puzzle[i]-1][puzzle[i+1]-1]);
        values_table[i][i+1].push_back(puzzle[i]);
        values_table[i][i+1].push_back(puzzle[i+1]);
        values_table[i][i+1].push_back(i+1);
    }
    ...
    for(int diagonal = 2; diagonal < puzzle_size; diagonal ++){
        ...
        for(int line = 0; line + diagonal < puzzle_size; line ++){
            ...
            for(int table_searcher = 1; table_searcher <= diagonal && (int) values_table[line][column].size() <= matrix_size; table_searcher++){
                ...
                for(int line_values_iterator = 0; line_values_iterator < (int) line_cell.size() && (int) values_table[line][column].size() <= matrix_size && !output; line_values_iterator+=4){

                    /* itera sobre os valores contidos na célula da mesma coluna */
                    for(int col_values_iterator = 0; col_values_iterator < (int) column_cell.size() && (int) values_table[line][column].size() <= matrix_size && !output; col_values_iterator+=4){
                        ...
                        if(!any_of(values_table[line][column].begin(), values_table[line][column].end(), [value](int x){ return x == value;})){
                            values_table[line][column].push_back(value);
                            //vector<int> &trace_info = values_table[line][column][1];
                            values_table[line][column].push_back(left_value);
                            values_table[line][column].push_back(right_value);
                            values_table[line][column].push_back(column - table_searcher + 1);
                        }
